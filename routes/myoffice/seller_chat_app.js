module.exports = function ( app ) {
    ...
    /*the charRecord*/
    app.get('/seller_chatMessage/:id?/:time',function(req,res){
    ...
    /*
    app.get('/seller_offline/:id',function(req,res){
        if(req.session.user){
            var id = req.params.id;
            var chatRcord = global.dbHelper.getModel('chatRcord');
            chatRcord.find({"order_id":id,"who":0,"tag":0},{"information":1,"userName":1,"_id":0},function (error, doc) {
                if (error) {
                    console.log(error);
                }else{
                    res.json(doc);
                    chatRcord.update({"order_id":id,"who":0,"tag":0},{$set:{"tag":1}},{multi:true},function (error, doc) {
                        if (error) {
                            console.log(error);
                        }else{
                            // console.log(doc);
    ...
    */
    ...
    /*chat history*/
    app.get('/seller_chatHistory/:id',function(req,res){
        if(req.session.user){
            var id = req.params.id;
            var chatRcord = global.dbHelper.getModel('chatRcord');
