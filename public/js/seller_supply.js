$(function() {
	...
	//侧栏颜色
	$('.wx-ullist:eq(2) li:eq(0) a').addClass('wx-click');
    ...
    /*关注信息总条数*/
    $.get("/seller_supply_count", function(data){
	    ...
	    $(".wx-page").createPage({
	        ...
	        backFn:function(p){
	            console.log(p); 
	            $.get("/seller_supply_page/"+p, function(data){
			        var str = "";
	    ...
	    /*页面加载时，获取到信息*/
	    $.get("/seller_supply_data", function(data){
	        var str = "";
	        for (var i = 0; i < data.length; i++) {
	            var date1 = new Date(data[i].jiaohuo_start).Format("yyyy-MM-dd");
	            var date2 = new Date(data[i].jiaohuo_end).Format("yyyy-MM-dd"); 
