$(function() {
    ...
    $.get("/sell_data", function(data){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var need =  data[i].needVolume - data[i].hasBuy;     
            ...
            str += "<td><a class='btn wx-sellGoods' href='javascript:void(0)'>售卖</a></td>";
            str = "<tr>"+str+"</tr>";
        };
        $('#wx-tbody').html(str);
    ...
    /*商品信息总条数*/
    $.get("/sell_count", function(data){
        localStorage.sellCount = Math.ceil(data/2);
    });

    // 分页js引入
    $(".wx-page:eq(0)").createPage({
        pageCount:localStorage.sellCount,
        current:1,
        backFn:function(p){
            // console.log(p); 
            $.get("/sell_data_page/"+p, function(data){
                var str = "";
                for (var i = 0; i < data.length; i++) {
                    var date1 = new Date(data[i].guadan_start).Format("yyyy-MM-dd");
                    var date2 = new Date(data[i].guadan_end).Format("yyyy-MM-dd"); 
    ...
    $('#wx-7').click(function () {
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                $(".wx-page:eq(1)").show();
                $(".wx-page:eq(0)").hide();
                localStorage.sellCount1 = Math.ceil(data/2);
                console.log(localStorage.sellCount1)
            }
        })
    })
    ...
    $(".wx-page:eq(1)").createPage({
        pageCount: localStorage.sellCount1,
        current:1,
        backFn:function(p){
            // console.log(p); 
            var search = new Object();
    ...
    $(document).on('click','.wx-sellGoods',function(){
        ...
        localStorage.sell_id = id;
        location.href = "sellDetail";
    })
