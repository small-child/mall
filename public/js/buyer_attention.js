$(function() {
	...
	//侧栏颜色
	$('.wx-ullist:eq(1) li:eq(1) a').addClass('wx-click');
	...
	/*关注信息总条数*/
    $.get("/buyer_attention_count", function(data){
        localStorage.buyer_attention_count = Math.ceil(data/2);
        // console.log(localStorage.buyer_attention_count);

        // 分页js引入
        $(".wx-page").createPage({
            pageCount:localStorage.buyer_attention_count,
            current:1,
            backFn:function(p){
                // console.log(p); 
                $.get("/buyer_attention_page/"+p, function(data){
                    var str = "";
                    for (var i = 0; i < data.length; i++) {
                        var date1 = new Date(data[i].jiaohuo_start).Format("yyyy-MM-dd");
                        var date2 = new Date(data[i].jiaohuo_end).Format("yyyy-MM-dd"); 
        ...
        $(document).on('click','.wx-deleteOrder',function(){
            ...
            $.get("/buyer_attention_delete/"+id, function(data){
                // console.log(data)
                if (data == 0) {
