$(function() {
    ...
    // 获取用户信息
    $.get("/admin_userIfo", function(data){
    	// console.log(data[0]);
        var str = "";
    ...
    /*商品信息总条数*/
    $.get("/admin_userIfo_count", function(data){
        // 分页js引入
        $(".wx-page").createPage({
            pageCount:Math.ceil(data/2),
            current:1,
            backFn:function(p){
                $.get("/admin_userIfo_page/"+p, function(data){
                    var str = "";
                    for (var i = 0; i < data.length; i++) {
                        var date = new Date(data[i].createDate).Format("yyyy-MM-dd hh:mm");
                        str += "<td>"+data[i]._id+"</td>";
    ...
    $(document).on('click','.wx-inspect',function(){
        ...
        // console.log(id);
        localStorage.admin_userId = id;
        location.href = "user_company";
    })
