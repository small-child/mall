$(function() {
	// console.log(localStorage.admin_inspectId);
    /*获取公司信息*/
	$.get("/admin/company_detail/"+localStorage.admin_inspectId, function(data){
    	if (data == 0) {
    		$('#wx-0').text("公司信息暂未提交!");
    ...
    /*通过验证*/
    $('#wx-21').click(function() {
        $.get("/admin/company_access/"+localStorage.admin_inspectId, function(data){
            if (data == 1) {
                $('#wx-22').text("认证成功");
                $('#wx-21').attr("disabled",true);
            } else{
